...
# Install USB mounter 
# ================================
## not in use? ## def install_usbmount():
## not in use? ##     ## done ##  result = sudo("apt-get install usbmount ntfs-3g -y","Unable to download usbmount")
## not in use? ##     ## done ##  display_log(result.output)
## not in use? ##     sudo("sed -i '/MountFlags=slave/c\MountFlags=shared' /lib/systemd/system/systemd-udevd.service","Unable to update udevd configuration (systemd-udevd.service)")
## not in use? ##     sudo("chmod +x ./build_elimupi/files/01_create_label_symlink ./build_elimupi/files/01_remove_model_symlink", False)
## not in use? ##     cp("./files/usbmount/usbmount.conf", "/etc/usbmount/")
## not in use? ##     cp("./files/usbmount/01_create_label_symlink", "/etc/usbmount/mount.d/")
## not in use? ##     cp("./files/usbmount/01_remove_model_symlink", "/etc/usbmount/umount.d")
## not in use? ##     cp("./files/usbmount/usbmount@.service", "/etc/systemd/system/")
## not in use? ##     cp("./files/usbmount/usbmount.rules", "/etc/udev/rules.d")
## not in use? ##     display_log("usb mount installed")
## not in use? ##     return True
...
## done ##    display_log("Completed configuring mariadb-server...", col_log_ok)
    ...
    ## done ##  display_log("Installing PHP and tools...", col_log_ok)
    ## done ##  sudo("apt-get install php-fpm php-mysql php -y","Unable to install php-fpm, php-mysql or php")
    ## done ##  display_log("Completed installing PHP and tools", col_log_ok)
    # /etc/php/7.3/fpm/php.ini

    # Install PHP extensions
    ## done ##  display_log("Installing PHP extensions...", col_log_ok)
    ## done ##  sudo("apt-get install php-curl php-xml php-mbstring php-zip php-gd php-intl php-xmlrpc php-soap -y","Unable to install PHP extensions")
    ## done ##  display_log("Done installing PHP extensions", col_log_ok)
...
def install_web_interface():
    ## done ##  display_log("Install nginx...")
    ## done ##  sudo("apt-get install -y nginx","Unable to install NGINX")  #INSTALL NGINX
    ## done ##  display_log("Install PHP extensions...", col_log_ok)
    ## done ##  sudo("apt install php-fpm -y","Unable to install NGINX")
...
def install_kolibri():
    ...
    ## done ##  sudo("apt install libffi-dev python3-pip python3-pkg-resources dirmngr -y", "Unable to install Kolibri step 1")
    ...
    ## done ##  display_log("Install Kolibri components...")
    ## done ##  sudo("apt install kolibri kolibri-server")
    ## done ##  display_log("Install Kolibri completed...", col_log_ok)
    return True
...
# ================================
def install_dnsmasq():
    ## done ##  sudo("apt-get install dnsmasq -y", "Unable to install dnsmasq.")
...
# ================================
def install_citadel():
    ## done ##  sudo("apt-get install citadel-suite -y", "Unable to install Citadel")
...
def install_fdroid():
    ...
    ## done ##  sudo("apt-get install fdroidserver -y", "Unable to install FDroid")
...
# ================================
## not in use? ## def setup_content_disk():
## not in use? ##     # Get all current disks
## not in use? ##     display_log("Get current disks\r\n")
## not in use? ##     result = sudo("lsblk -Jno mountpoint,label","")
## not in use? ##     disk_info = json.loads(result.output)
## not in use? ##     for item in disk_info["blockdevices"]:
## not in use? ##         if item["label"] is not None:
## not in use? ##             # print("Label: " + item["label"])
## not in use? ##             display_log("Label : " + str(item["label"]) + "\r\n")
## not in use? ##     display_log("\r\n----------------------------------\r\n")
## not in use? ##     sys.exit(0)
## not in use? ##     return True
...
## not in use? ## def create_img():
## not in use? ##     # use scripts to create an gzipped, compressed image file to distribute on the external disk
## not in use? ##     sudo("mkdir /mnt/","Unable to create image folder")
## not in use? ##     return True
# ================================
# Sudo 
# ================================
## will not be used ## def sudo(s, error_msg = False):
## will not be used ##     result = cmd("sudo DEBIAN_FRONTEND=noninteractive %s" % s)
## will not be used ##     if error_msg and not result.result:
## will not be used ##         die(error_msg, result)
## will not be used ##     return result 

# ================================
# abort command (clean exit)
# ================================
## will not be used ## def abort(msg):
## will not be used ##     curses.endwin()
## will not be used ##     print("Exit: " + str(msg))
## will not be used ##     sys.exit(0)
    
# ================================
# die command (error exit)
# ================================
## will not be used ## def die(msg, cmd_result=None):
## will not be used ##     # End cusrses mode
## will not be used ##     curses.endwin()
## will not be used ##     
## will not be used ##     # display error
## will not be used ##     print("Error: " + str(msg))
## will not be used ##     if cmd_result:
## will not be used ##         print(cmd_result.error)
## will not be used ##     sys.exit(1)

# ================================
# CMD command
# ================================
## will not be used ## def cmd(run_cmd):
## will not be used ##     # initialize result
## will not be used ##     cmd_result=CmdResult()
## will not be used ##     # Execute command, PY 2.4 - 2.6
## will not be used ##     result = subprocess.Popen(run_cmd, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)
## will not be used ##     # result = subprocess.check_output(c, shell=True, stdin=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)
## will not be used ##     try:
## will not be used ##         cmd_result.output, cmd_result.error = result.communicate() # PY 2.4 - 2.6
## will not be used ##     except KeyboardInterrupt:
## will not be used ##         pass
## will not be used ##     cmd_result.result = (result.returncode == 0)
## will not be used ##     #return (result.returncode == 0)
## will not be used ##     return (cmd_result)

# ================================
# exists command
# ================================
## will not be used ## def exists(p):
## will not be used ##     return os.path.isfile(p) or os.path.isdir(p)

# ================================
# Copy command
# ================================    
## will not be used ## def cp(source_file, destination_file, err_msg=False):
## will not be used ##     if source_file.startswith("/"):
## will not be used ##         sudo("cp {} {}".format(source_file, destination_file), err_msg)
## will not be used ##     elif localinstaller():
## will not be used ##         sudo("cp %s/%s %s" % (basedir(), source_file, destination_file), err_msg)
## will not be used ##     else:
## will not be used ##         sudo("cp %s/%s %s" % (basedir() + "/build_elimupi", source_file, destination_file), err_msg)

# ================================
# Basedir command
# ================================
## will not be used ## def basedir():
## will not be used ##     bindir = os.path.dirname(os.path.realpath(sys.argv[0]))     # Should be initial folder where install is started 
## will not be used ##     if not bindir:
## will not be used ##         bindir = "."
## will not be used ##     else:
## will not be used ##         return bindir

## will not be used ## def localinstaller():
## will not be used ##     if exists( basedir() + "/files"):
## will not be used ##         return True
## will not be used ##     else:
## will not be used ##         return False 

# ================================
# check if is virtual environment 
# ================================
## why is vagrant an option ## def is_vagrant():
## why is vagrant an option ##     return os.path.isfile("/etc/is_vagrant_vm")

# ================================
# Popup window for Yes or No
# ================================
## will not be used ## def yes_or_no(question, row = 1, col = 2):
## will not be used ##     statwin.addstr(row, col, question+' (y/n): ', col_info)
## will not be used ##     statwin.refresh()
## will not be used ##     curses.curs_set(True)
## will not be used ##     try:
## will not be used ##         while True:
## will not be used ##             char = statwin.getch()
## will not be used ##             if char > 0:
## will not be used ##                 if chr(char) == 'y':
## will not be used ##                     curses.curs_set(False)
## will not be used ##                     return True
## will not be used ##                 if chr(char) == 'n':
## will not be used ##                     curses.curs_set(False)
## will not be used ##                     return False
## will not be used ##     except KeyboardInterrupt:
## will not be used ##         die("interrupted...")
        
# ================================
# Home directory
# ================================
## not in use ## def homedir():
## not in use ##     home = os.path.expanduser("~")
## not in use ##     return home
## not in use ## 
# ================================
...
# ================================
def wifi_present():
    ## why is vagrant an option ## if is_vagrant():
    ## why is vagrant an option ##     return False
...
def PHASE0():
    ...
    # Get latest GIT
    # ================================
    ## done ##  statwin.addstr( 3,3, "?" , col_info)
    ## done ##  statwin.refresh()
    ## done ##  result = sudo("apt-get install -y git", "Unable to install Git.")
    ## done ##  display_log(result.output)
    ## done ##  statwin.addstr( 3,3, "*" , col_info_ok)
    ## done ##  statwin.refresh()
    ...
    # Set password
    # ================================
    ## why is vagrant an option ## if not is_vagrant():
